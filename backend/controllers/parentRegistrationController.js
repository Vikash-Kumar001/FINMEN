    ...
    if (flow === 'parent_exists' && parentId) {
      ...
      if (selectedPlan && selectedPlan !== 'free') {
        ...
        await UserSubscription.create({
          ...
          renewalAmount: renewalAmounts[selectedPlan],
          isFirstYear: true,
          status: 'active',
          startDate: new Date(),
  ...
  try {
    ...
    if (registrationIntent.selectedPlan !== 'free') {
      await UserSubscription.create({
        ...
        firstYearAmount: planAmounts[registrationIntent.selectedPlan],
        renewalAmount: registrationIntent.selectedPlan === 'student_premium' ? 999 : 1499,
        isFirstYear: true,
        status: 'active',
        startDate: new Date(),
