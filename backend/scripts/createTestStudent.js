...
const createTestStudent = async () => {
  try {
    ...
    const testEmail = 'teststudent@finmen.com';
    const testPassword = 'TestStudent123!';
    ...
    if (existingStudent) {
      ...
      if (subscription) {
        ...
        // Update to student_premium if not already
        if (subscription.planType !== 'student_premium') {
          console.log('🔄 Updating subscription to student_premium...');
          subscription.planType = 'student_premium';
          subscription.planName = 'Students Premium Plan';
          subscription.features = STUDENT_PREMIUM_FEATURES;
          subscription.amount = 4499;
          subscription.firstYearAmount = 4499;
          subscription.renewalAmount = 999;
          await subscription.save();
          console.log('✅ Subscription updated to student_premium');
        }
      } else {
        console.log('⚠️ No active subscription found. Creating student_premium subscription...');
    ...
    // Create subscription with student_premium plan
    ...
    console.log('   Password:', testPassword);
    console.log('   Linking Code:', linkingCode);
    console.log('   Plan: student_premium');
...
const createSubscription = async (studentId) => {
  try {
    ...
    if (existingSubscription && existingSubscription.planType === 'student_premium') {
      console.log('✅ Active student_premium subscription already exists');
    ...
    const subscription = await UserSubscription.create({
      ...
      startDate: new Date(),
      endDate: endDate,
      features: STUDENT_PREMIUM_FEATURES,
      transactions: [{
        transactionId: `test_student_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`,
