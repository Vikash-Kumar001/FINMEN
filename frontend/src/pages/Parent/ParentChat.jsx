...
const ParentChat = () => {
  ...
  // Use childId if available, otherwise use studentId
  const actualStudentId = childId || studentId;
  
  console.log('ParentChat params:', { studentId, childId, actualStudentId });
  ...
  const fetchChat = useCallback(async () => {
    try {
      setLoading(true);
      console.log('Fetching chat for student:', actualStudentId);
      
      // Validate that actualStudentId is a valid ObjectId
      if (!actualStudentId || actualStudentId === 'progress' || actualStudentId.length !== 24) {
        throw new Error('Invalid student ID. Please check the URL and try again.');
      }
  ...
  useEffect(() => {
    ...
    if (!actualStudentId || actualStudentId === 'progress' || actualStudentId.length !== 24) {
      console.error('Invalid student ID:', actualStudentId);
      toast.error('Invalid student ID. Please check the URL and try again.');
      navigate('/parent/children');
      return;
    }
      fetchChat();
  }, [actualStudentId, fetchChat, navigate]);
  ...
  if (!actualStudentId || actualStudentId === 'progress' || actualStudentId.length !== 24) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">Invalid student ID. Please check the URL and try again.</p>
          <p className="text-gray-600 mb-4">Current URL: {window.location.pathname}</p>
          <button
              ...
              <div className="relative">
                <div className="absolute top-3 right-3 z-10">
                  <button
                    ...
                    className="p-1.5 text-gray-500 hover:text-gray-700 rounded-full hover:bg-gray-200 transition-colors cursor-pointer bg-white shadow-md"
