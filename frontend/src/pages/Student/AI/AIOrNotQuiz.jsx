...
const AIOrNotQuiz = () => {
  ...
  const optionButtonStyle = (option) => {
    ...
    return {
      ...
      borderRadius: '20px',
      backgroundColor: option === 'AI' ? '#4CAF50' : '#F44336',
      border: `4px solid ${isCorrectFeedback ? '#2E7D32' : isWrongFeedback ? '#C62828' : option === 'AI' ? '#388E3C' : '#D32F2F'}`,
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentItem.correctAnswer) {
      ...
      setScore(prevScore => prevScore + currentItem.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentItem.correctAnswer}`,
        type: 'wrong'
      });
  ...
  return (
    <GameShell title="AI or Not Quiz" subtitle="Decide what is AI and what is not">
      ...
      <div style={optionsContainerStyle}>
        ...
        <button
          style={optionButtonStyle('Not AI')}
          onClick={() => handleOptionClick('Not AI')}
          disabled={isOptionDisabled}
        >
          NOT AI ❌
        </button>
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{items.length}</span>
      </div>
      <style>{`
        @keyframes pop-in {
