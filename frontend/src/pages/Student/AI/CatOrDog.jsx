...
const CatOrDog = () => {
  ...
  const images = [
    { id: 1, image: '🐱', correctAnswer: 'cat', rewardPoints: 5 },
    { id: 2, image: '🐶', correctAnswer: 'dog', rewardPoints: 5 },
    { id: 3, image: '🐱', correctAnswer: 'cat', rewardPoints: 5 },
    { id: 4, image: '🐶', correctAnswer: 'dog', rewardPoints: 5 },
    { id: 5, image: '🐱', correctAnswer: 'cat', rewardPoints: 5 },
  ];
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentImage.correctAnswer) {
      ...
      setScore(prevScore => prevScore + currentImage.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentImage.correctAnswer}`,
        type: 'wrong'
      });
  ...
  return (
    <GameShell title="Cat or Dog" subtitle="Pick the right animal!">
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-6 mb-6 border border-white/20 shadow-2xl z-10 max-w-3xl mx-auto w-full">
        <div style={imageStyle}>{currentImage.image}</div>
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{images.length}</span>
      </div>
      <style>{`
        @keyframes pop-in {
