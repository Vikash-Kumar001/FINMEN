...
const PatternFindingPuzzle = () => {
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentPuzzle.correctAnswer) {
      ...
      setScore(prevScore => prevScore + currentPuzzle.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentPuzzle.correctAnswer}`,
        type: 'wrong'
      });
  ...
  return (
    <GameShell title="Pattern Finding Puzzle" subtitle="Find the next in the sequence">
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{puzzles.length}</span>
      </div>
      <style>{`
        @keyframes pop-in {
