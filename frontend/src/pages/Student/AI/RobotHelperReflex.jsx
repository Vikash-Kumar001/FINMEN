...
const RobotHelperReflex = () => {
  ...
  const reflexes = [
    { 
      id: 1, 
      scenario: '🚨 Emergency! What should robot do first?', 
      correctAnswer: 'call for help', 
      options: ['call for help', 'run away', 'ignore', 'laugh'],
      rewardPoints: 5 
    },
    { 
      id: 2, 
      scenario: '🔥 Fire detected! What should robot do?', 
      correctAnswer: 'alert everyone', 
      options: ['alert everyone', 'hide', 'ignore', 'laugh'],
      rewardPoints: 5 
    },
    { 
      id: 3, 
      scenario: '🚑 Someone is hurt! What should robot do?', 
      correctAnswer: 'get help', 
      options: ['get help', 'run away', 'ignore', 'laugh'],
      rewardPoints: 5 
    },
    { 
      id: 4, 
      scenario: '🚪 Door is stuck! What should robot do?', 
      correctAnswer: 'try to open', 
      options: ['try to open', 'give up', 'ignore', 'laugh'],
      rewardPoints: 5 
    },
    { 
      id: 5, 
      scenario: '📞 Phone is ringing! What should robot do?', 
      correctAnswer: 'answer it', 
      options: ['answer it', 'ignore it', 'break it', 'laugh'],
      rewardPoints: 5 
    },
  ];
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentReflex.correctAnswer) {
      ...
      setScore(prevScore => prevScore + currentReflex.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentReflex.correctAnswer}`,
        type: 'wrong'
      });
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* Title */}
      <div style={titleBoxStyle}>Robot Helper Reflex</div>

      {/* Game Card */}
      <div style={gameCardStyle}>
        <div style={scenarioStyle}>
          {currentReflex.scenario}
        </div>
      </div>

      {/* Options */}
      <div style={optionsContainerStyle}>
        {currentReflex.options.map((option, index) => (
          <button
            key={index}
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{reflexes.length}</span>
      </div>
