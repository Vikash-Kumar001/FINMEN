...
const SmartHomeStory = () => {
  ...
  const scenarios = [
    { 
      id: 1, 
      scenario: '🏠 Smart home detects you coming home. What should it do?', 
      correctAnswer: 'turn on lights', 
      options: ['turn on lights', 'turn off lights', 'call police', 'do nothing'],
      rewardPoints: 5 
    },
    { 
      id: 2, 
      scenario: '🌡️ Smart home sees it\'s getting cold. What should it do?', 
      correctAnswer: 'turn on heat', 
      options: ['turn on heat', 'turn on AC', 'open windows', 'do nothing'],
      rewardPoints: 5 
    },
    { 
      id: 3, 
      scenario: '🔒 Smart home detects someone at the door. What should it do?', 
      correctAnswer: 'ring doorbell', 
      options: ['ring doorbell', 'open door', 'call police', 'do nothing'],
      rewardPoints: 5 
    },
    { 
      id: 4, 
      scenario: '🌙 Smart home sees it\'s bedtime. What should it do?', 
      correctAnswer: 'dim lights', 
      options: ['dim lights', 'turn on lights', 'play music', 'do nothing'],
      rewardPoints: 5 
    },
    { 
      id: 5, 
      scenario: '☔ Smart home detects rain. What should it do?', 
      correctAnswer: 'close windows', 
      options: ['close windows', 'open windows', 'turn on fan', 'do nothing'],
      rewardPoints: 5 
    },
  ];
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentScenario.correctAnswer) {
      setFeedback({ message: 'Great! Smart home working!', type: 'correct' });
      setScore(prevScore => prevScore + currentScenario.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentScenario.correctAnswer}`,
        type: 'wrong'
      });
  ...
  return (
    <GameShell title="Smart Home Story" subtitle="Choose what a smart home should do">
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{scenarios.length}</span>
      </div>
