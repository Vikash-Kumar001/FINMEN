...
const SortingAnimals = () => {
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentAnimal.correctAnswer) {
      ...
      setScore(prevScore => prevScore + currentAnimal.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentAnimal.correctAnswer}`,
        type: 'wrong'
      });
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* Title */}
      <div style={titleBoxStyle}>Sorting Animals</div>

      {/* Game Card */}
      <div style={gameCardStyle}>
        <div style={animalStyle}>{currentAnimal.animal}</div>
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{animals.length}</span>
      </div>
