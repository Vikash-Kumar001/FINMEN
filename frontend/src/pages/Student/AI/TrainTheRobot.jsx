...
const TrainTheRobot = () => {
  ...
  const trainingTasks = [
    { 
      id: 1, 
      task: '🤖 Robot needs to learn to wave. What should you do?', 
      correctAnswer: 'show it how', 
      options: ['show it how', 'tell it to stop', 'ignore it', 'yell at it'],
      rewardPoints: 5 
    },
    { 
      id: 2, 
      task: '🤖 Robot makes a mistake. What should you do?', 
      correctAnswer: 'help it learn', 
      options: ['help it learn', 'get angry', 'ignore it', 'turn it off'],
      rewardPoints: 5 
    },
    { 
      id: 3, 
      task: '🤖 Robot does something right. What should you do?', 
      correctAnswer: 'praise it', 
      options: ['praise it', 'ignore it', 'get angry', 'turn it off'],
      rewardPoints: 5 
    },
    { 
      id: 4, 
      task: '🤖 Robot is confused. What should you do?', 
      correctAnswer: 'be patient', 
      options: ['be patient', 'get angry', 'ignore it', 'turn it off'],
      rewardPoints: 5 
    },
    { 
      id: 5, 
      task: '🤖 Robot learns something new. What should you do?', 
      correctAnswer: 'celebrate', 
      options: ['celebrate', 'ignore it', 'get angry', 'turn it off'],
      rewardPoints: 5 
    },
  ];

  const currentTask = trainingTasks[currentLevelIndex];
  ...
  const handleOptionClick = (option) => {
    ...
    if (option === currentTask.correctAnswer) {
      setFeedback({ message: 'Great! Good training!', type: 'correct' });
      setScore(prevScore => prevScore + currentTask.rewardPoints);
      setShowConfetti(true);
    } else {
      setFeedback({
        message: `Wrong! The correct answer is: ${currentTask.correctAnswer}`,
        type: 'wrong'
      });
  ...
  const handleNextLevel = () => {
    setShowConfetti(false);
    if (currentLevelIndex < trainingTasks.length - 1) {
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* Title */}
      <div style={titleBoxStyle}>Train the Robot</div>
      ...
      <div style={scoreTrackerStyle}>
        Score: {score}
        <span style={{ marginLeft: '10px' }}>⭐ {currentLevelIndex + 1}/{trainingTasks.length}</span>
      </div>
