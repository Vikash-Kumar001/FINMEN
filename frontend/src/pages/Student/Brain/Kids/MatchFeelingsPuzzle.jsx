// File: MatchFeelingsPuzzle.js
import React, { useState, useEffect } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';
import GameShell, { GameCard, FeedbackBubble } from '../../Finance/GameShell';
import { Brain, Smile, Frown, Zap, Sparkles, Timer, Trophy, RotateCcw } from 'lucide-react';
...
const MatchFeelingsPuzzle = () => {
  ...
  // Feelings icons with labels
  const feelings = [
    { Icon: Smile, label: "Happy", color: "bg-yellow-400" },
    { Icon: Frown, label: "Sad", color: "bg-blue-400" },
    { Icon: Zap, label: "Angry", color: "bg-red-400" },
    { Icon: Sparkles, label: "Excited", color: "bg-purple-400" },
    { Icon: Smile, label: "Happy", color: "bg-yellow-400" },
    { Icon: Frown, label: "Sad", color: "bg-blue-400" },
    { Icon: Zap, label: "Angry", color: "bg-red-400" },
    { Icon: Sparkles, label: "Excited", color: "bg-purple-400" }
  ...
  useEffect(() => {
    ...
    // Create pairs of feelings
    let levelFeelings = [];
    for (let i = 0; i < numPairs; i++) {
      const feeling = feelings[i % 4];
      levelFeelings.push({ id: i * 2, ...feeling, type: i });
      levelFeelings.push({ id: i * 2 + 1, ...feeling, type: i });
    }
    
    // Shuffle the cards
    const shuffledFeelings = [...levelFeelings].sort(() => Math.random() - 0.5);
    
    setCards(shuffledFeelings.map(card => ({ ...card, isFlipped: false, isMatched: false })));
    setFlipped([]);
    setMatched([]);
    setMoves(0);
  ...
  const handleFlip = (id) => {
    ...
    if (newFlipped.length === 2) {
      ...
      if (firstCard.type === secondCard.type) {
        ...
        setFeedbackMessage(`Feelings matched! +${10 + streak * 2} points`);
        setShowFeedback(true);
  ...
  return (
    <GameShell
      title="Feelings Memory Puzzle"
      score={score}
      currentLevel={currentLevel}
      totalLevels={5}
      coinsPerLevel={coinsPerLevel}
      gameId="brain-kids-84"
        ...
        {levelCompleted && (
          ...
          >
            ...
            <div className="bg-gradient-to-r from-yellow-500/30 to-orange-500/30 rounded-xl p-4 mb-6 border-2 border-yellow-400">
              ...
              <div className="text-xl font-bold text-white mb-4">Feelings Expert Badge</div>
              <p className="text-white/90">
                {score >= 100 ? "🏆 Emotion Champion!" : 
                 score >= 80 ? "🥇 Feelings Expert!" : 
                 score >= 60 ? "🥈 Good Job!" : 
                 "🥉 Keep Practicing!"}
              </p>
            </div>
