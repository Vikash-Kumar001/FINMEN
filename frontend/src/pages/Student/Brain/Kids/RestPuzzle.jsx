...
import { Brain, Moon, Tv, Battery, Frown, Timer, Trophy, RotateCcw } from 'lucide-react';
...
const RestPuzzle = () => {
  ...
  // Rest habits with icons
  const restHabits = [
    { Icon: Moon, label: "Nap", effect: "Energy", color: "bg-indigo-400" },
    { Icon: Battery, label: "Energy", effect: "Nap", color: "bg-green-400" },
    { Icon: Tv, label: "All-night TV", effect: "Tired", color: "bg-red-400" },
    { Icon: Frown, label: "Tired", effect: "All-night TV", color: "bg-orange-400" },
    { Icon: Moon, label: "Nap", effect: "Energy", color: "bg-indigo-400" },
    { Icon: Battery, label: "Energy", effect: "Nap", color: "bg-green-400" },
    { Icon: Tv, label: "All-night TV", effect: "Tired", color: "bg-red-400" },
    { Icon: Frown, label: "Tired", effect: "All-night TV", color: "bg-orange-400" }
  ...
  useEffect(() => {
    ...
    // Create pairs of rest habits
    let levelHabits = [];
    for (let i = 0; i < numPairs; i++) {
      const habit = restHabits[i % 4];
      levelHabits.push({ id: i * 2, ...habit, type: i });
      levelHabits.push({ id: i * 2 + 1, ...restHabits.find(h => h.label === habit.effect), type: i });
    }
    
    // Shuffle the cards
    const shuffledHabits = [...levelHabits].sort(() => Math.random() - 0.5);
    
    setCards(shuffledHabits.map(card => ({ ...card, isFlipped: false, isMatched: false })));
    setFlipped([]);
    setMatched([]);
    setMoves(0);
  ...
  return (
    <GameShell
      title="Rest Habits Memory Puzzle"
      score={score}
      currentLevel={currentLevel}
      totalLevels={5}
      coinsPerLevel={coinsPerLevel}
      gameId="brain-kids-124"
      ...
      totalXp={totalXp}>
      <GameCard>
        <h3 className="text-2xl font-bold text-white mb-2 text-center">Rest Habits Memory Puzzle</h3>
        <p className="text-white/80 mb-4 text-center">Match rest habits with their effects!</p>
        ...
        {levelCompleted && (
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-white mb-2">Rest Master!</h3>
            <p className="text-white/80 mb-4">You've mastered rest habits with {score} points!</p>
            ...
            <div className="bg-gradient-to-r from-yellow-500/30 to-orange-500/30 rounded-xl p-4 mb-6 border-2 border-yellow-400">
              ...
              <div className="text-xl font-bold text-white mb-4">Rest Habits Expert Badge</div>
              <p className="text-white/90">
                {score >= 100 ? "🏆 Rest Champion!" : 
                 score >= 80 ? "🥇 Sleep Expert!" : 
                 score >= 60 ? "🥈 Good Job!" : 
                 "🥉 Keep Practicing!"}
              </p>
            </div>
