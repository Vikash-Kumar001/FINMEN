...
const FootprintBadge = () => {
  ...
  const handleCompleteTask = () => {
    if (!completedTasks.includes(postingActs[currentTask].id)) {
      const newCompleted = [...completedTasks, postingActs[currentTask].id];
      setCompletedTasks(newCompleted);
      showCorrectAnswerFeedback(1, true);
      
      if (newCompleted.length === postingActs.length) {
        setTimeout(() => {
          setShowBadge(true);
        }, 500);
      } else {
        setTimeout(() => {
          setCurrentTask(prev => (prev + 1) % postingActs.length);
        }, 500);
      }
    }
  };

  const currentAct = postingActs[currentTask];
  ...
  return (
    <GameShell
      ...
      subtitle={!showBadge ? `Task ${completedTasks.length + 1} of ${postingActs.length}` : "Badge Earned!"}
      ...
      gameId={gameId}
      gameType="dcos"
      totalLevels={postingActs.length}
      currentLevel={completedTasks.length + 1}
      maxScore={postingActs.length}
        ...
        {!showBadge ? (
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 md:p-8 border border-white/20 w-full max-w-2xl">
            ...
            <p className="text-white/80 mb-6 text-center">
              Complete these safe posting acts to earn your badge!
            </p>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 md:p-8 border border-white/20 w-full max-w-2xl text-center">
            <div className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 text-white rounded-2xl p-8 text-center animate-pulse">
              ...
              <p className="text-lg md:text-xl mb-4">
                You've earned the <strong>Smart Poster Badge!</strong> 🌟
              </p>
              <p className="text-white/90 text-sm">
                Great job! You're a smart and kind digital citizen. Keep spreading positivity online!
              </p>
            </div>
          </div>
        )}
