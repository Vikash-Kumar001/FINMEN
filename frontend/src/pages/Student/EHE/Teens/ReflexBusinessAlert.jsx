...
const ReflexBusinessAlert = () => {
  ...
  const scenarios = [
    { action: "Budget Plan", emoji: "📊", isCorrect: true },
    { action: "Random Spend", emoji: "🎲", isCorrect: false },
    { action: "Track Expenses", emoji: "🧾", isCorrect: true },
    { action: "Ignore Costs", emoji: "🙈", isCorrect: false },
    { action: "Save Money", emoji: "💰", isCorrect: true },
    { action: "Overspend", emoji: "💸", isCorrect: false },
    { action: "Plan Purchases", emoji: "📝", isCorrect: true },
    { action: "Impulse Buy", emoji: "🛒", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for business decisions...'}
              {gameState === 'showing' && 'Watch for the financial signal...'}
              {gameState === 'ready' && 'QUICK! Choose the smart business approach!'}
              {gameState === 'success' && `Great decision! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Choose smart business practices!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Smart Business Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Poor Business Decision!</h3>
                <p className="text-white/90">Remember: Choose smart financial practices!</p>
              </div>
            </div>
          )}
        </div>
