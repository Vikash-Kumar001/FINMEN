...
const ReflexCareerAlert = () => {
  ...
  const scenarios = [
    { action: "Adapt & Learn", emoji: "🚀", isCorrect: true },
    { action: "Stay Stuck", emoji: "🛑", isCorrect: false },
    { action: "Embrace Change", emoji: "🔄", isCorrect: true },
    { action: "Resist Growth", emoji: "🛡️", isCorrect: false },
    { action: "Seek Opportunities", emoji: "🔍", isCorrect: true },
    { action: "Avoid Challenges", emoji: "😴", isCorrect: false },
    { action: "Upgrade Skills", emoji: "📈", isCorrect: true },
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for career alert decisions...'}
              {gameState === 'showing' && 'Watch for the career signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right career action!'}
              {gameState === 'success' && `Great alertness! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Career success needs quick thinking!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Career Decision!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Career Alert!</h3>
                <p className="text-white/90">Remember: Career success requires proactive, quick decisions!</p>
              </div>
            </div>
          )}
        </div>
