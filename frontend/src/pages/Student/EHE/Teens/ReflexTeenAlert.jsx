...
const ReflexTeenAlert = () => {
  ...
  const scenarios = [
    { action: "Adapt to Change", emoji: "🔄", isCorrect: true },
    { action: "Refuse Learning", emoji: "❌", isCorrect: false },
    { action: "Embrace New Tech", emoji: "💻", isCorrect: true },
    { action: "Ignore Trends", emoji: "🙈", isCorrect: false },
    { action: "Continuous Growth", emoji: "📈", isCorrect: true },
    { action: "Stay Static", emoji: "⏸️", isCorrect: false },
    { action: "Learn Skills", emoji: "📚", isCorrect: true },
    { action: "Avoid Challenges", emoji: "😴", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for future readiness decisions...'}
              {gameState === 'showing' && 'Watch for the alert signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right future readiness action!'}
              {gameState === 'success' && `Great alertness! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Future readiness needs quick thinking!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Alertness!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Alert!</h3>
                <p className="text-white/90">Remember: Future readiness requires quick, thoughtful decisions!</p>
              </div>
            </div>
          )}
        </div>
