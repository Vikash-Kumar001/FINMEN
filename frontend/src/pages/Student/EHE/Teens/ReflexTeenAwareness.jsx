...
const ReflexTeenAwareness = () => {
  ...
  const scenarios = [
    { action: "Scholarships Help", emoji: "💸", isCorrect: true },
    { action: "Give Up on Dreams", emoji: "🏳️", isCorrect: false },
    { action: "Research Opportunities", emoji: "🔍", isCorrect: true },
    { action: "Ignore Options", emoji: "🙈", isCorrect: false },
    { action: "Plan Financially", emoji: "📊", isCorrect: true },
    { action: "Spend Recklessly", emoji: "💳", isCorrect: false },
    { action: "Seek Mentorship", emoji: "👨‍🏫", isCorrect: true },
    { action: "Avoid Guidance", emoji: "🤐", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for educational awareness decisions...'}
              {gameState === 'showing' && 'Watch for the awareness signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right educational awareness action!'}
              {gameState === 'success' && `Great awareness! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Educational awareness needs quick thinking!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Awareness!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Awareness!</h3>
                <p className="text-white/90">Remember: Good educational awareness requires quick, thoughtful decisions!</p>
              </div>
            </div>
          )}
        </div>
