...
const ReflexTeenBoss = () => {
  ...
  const scenarios = [
    { action: "Learn & Grow", emoji: "📈", isCorrect: true },
    { action: "Stay Idle", emoji: "🛋️", isCorrect: false },
    { action: "Take Feedback", emoji: "💬", isCorrect: true },
    { action: "Ignore Advice", emoji: "🙉", isCorrect: false },
    { action: "Plan Ahead", emoji: "📝", isCorrect: true },
    { action: "Act Randomly", emoji: "🎲", isCorrect: false },
    { action: "Lead Team", emoji: "👥", isCorrect: true },
    { action: "Work Alone", emoji: "👤", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for leadership decisions...'}
              {gameState === 'showing' && 'Watch for the boss signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right leadership action!'}
              {gameState === 'success' && `Great leadership! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Leaders act decisively!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Leadership!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Leadership Opportunity!</h3>
                <p className="text-white/90">Remember: Effective leaders act decisively and thoughtfully!</p>
              </div>
            </div>
          )}
        </div>
