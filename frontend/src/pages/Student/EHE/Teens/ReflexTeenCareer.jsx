...
const ReflexTeenCareer = () => {
  ...
  const scenarios = [
    { action: "Lawyer = Court", emoji: "⚖️", isCorrect: true },
    { action: "Lawyer = Hospital", emoji: "🏥", isCorrect: false },
    { action: "Chef = Kitchen", emoji: "👨‍🍳", isCorrect: true },
    { action: "Chef = Classroom", emoji: "📚", isCorrect: false },
    { action: "Pilot = Airplane", emoji: "✈️", isCorrect: true },
    { action: "Pilot = Ship", emoji: "🚢", isCorrect: false },
    { action: "Doctor = Hospital", emoji: "👨‍⚕️", isCorrect: true },
    { action: "Doctor = Court", emoji: "⚖️", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for career matching...'}
              {gameState === 'showing' && 'Watch for the career signal...'}
              {gameState === 'ready' && 'QUICK! Match the career to the right place!'}
              {gameState === 'success' && `Great match! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Careers need correct matching!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Correct Match!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Wrong Match!</h3>
                <p className="text-white/90">Remember: Match careers to their correct workplaces!</p>
              </div>
            </div>
          )}
        </div>
