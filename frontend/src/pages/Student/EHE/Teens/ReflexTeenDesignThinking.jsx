...
const ReflexTeenDesignThinking = () => {
  ...
  const scenarios = [
    { action: "Improve After Feedback", emoji: "🔄", isCorrect: true },
    { action: "Never Change Idea", emoji: "🔒", isCorrect: false },
    { action: "Test with Users", emoji: "🧪", isCorrect: true },
    { action: "Ignore All Input", emoji: "🔇", isCorrect: false },
    { action: "Iterate Solution", emoji: "🔁", isCorrect: true },
    { action: "Stick to First Draft", emoji: "📝", isCorrect: false },
    { action: "Refine Based on Data", emoji: "📊", isCorrect: true },
    { action: "Reject All Criticism", emoji: "🙅", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for design thinking choices...'}
              {gameState === 'showing' && 'Watch for the design thinking signal...'}
              {gameState === 'ready' && 'QUICK! Choose the design thinking approach!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Choose design thinking principles!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Design Thinking Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Design Thinking!</h3>
                <p className="text-white/90">Remember: Choose iterative and user-centered approaches!</p>
              </div>
            </div>
          )}
        </div>
