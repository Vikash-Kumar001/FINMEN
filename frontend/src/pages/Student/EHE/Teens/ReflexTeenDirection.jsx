...
const ReflexTeenDirection = () => {
  ...
  const scenarios = [
    { action: "Career Planning", emoji: "🗺️", isCorrect: true },
    { action: "No Planning", emoji: "🧭", isCorrect: false },
    { action: "Set Goals", emoji: "🏁", isCorrect: true },
    { action: "Wander Aimlessly", emoji: "🚶", isCorrect: false },
    { action: "Research Options", emoji: "🔍", isCorrect: true },
    { action: "Make Random Choices", emoji: "🎲", isCorrect: false },
    { action: "Seek Guidance", emoji: "👥", isCorrect: true },
    { action: "Ignore Advice", emoji: "🙉", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for career direction decisions...'}
              {gameState === 'showing' && 'Watch for the direction signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right career direction!'}
              {gameState === 'success' && `Great direction! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Career direction needs quick thinking!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Direction!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Direction!</h3>
                <p className="text-white/90">Remember: Good career direction requires quick, thoughtful decisions!</p>
              </div>
            </div>
          )}
        </div>
