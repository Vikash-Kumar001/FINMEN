...
const ReflexTeenEntrepreneur = () => {
  ...
  const scenarios = [
    { action: "Start Early", emoji: "🚀", isCorrect: true },
    { action: "Wait Forever", emoji: "⏰", isCorrect: false },
    { action: "Take Action", emoji: "💪", isCorrect: true },
    { action: "Procrastinate", emoji: "😴", isCorrect: false },
    { action: "Learn Skills", emoji: "📚", isCorrect: true },
    { action: "Stay Ignorant", emoji: "❓", isCorrect: false },
    { action: "Build Network", emoji: "🤝", isCorrect: true },
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for entrepreneurial decisions...'}
              {gameState === 'showing' && 'Watch for the entrepreneurial signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right entrepreneurial action!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Entrepreneurs act quickly!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Successful entrepreneurs act decisively!</p>
              </div>
            </div>
          )}
        </div>
