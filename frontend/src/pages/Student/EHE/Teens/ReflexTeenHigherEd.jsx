...
const ReflexTeenHigherEd = () => {
  ...
  const scenarios = [
    { action: "Choose Based on Interests", emoji: "🎓", isCorrect: true },
    { action: "Copy Friends", emoji: "👥", isCorrect: false },
    { action: "Research Thoroughly", emoji: "🔍", isCorrect: true },
    { action: "Guess Randomly", emoji: "🎲", isCorrect: false },
    { action: "Plan Ahead", emoji: "📅", isCorrect: true },
    { action: "Procrastinate", emoji: "⏰", isCorrect: false },
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for higher education decisions...'}
              {gameState === 'showing' && 'Watch for the education signal...'}
              {gameState === 'ready' && 'QUICK! Choose the right higher education action!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Higher education planning needs quick thinking!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Decision!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Opportunity!</h3>
                <p className="text-white/90">Remember: Good higher education planning requires quick, thoughtful decisions!</p>
              </div>
            </div>
          )}
        </div>
