...
const ReflexTeenInnovator = () => {
  ...
  const scenarios = [
    { action: "Creative Ideas", emoji: "💡", isCorrect: true },
    { action: "Copy Others", emoji: "📎", isCorrect: false },
    { action: "Solve Problems", emoji: "🧩", isCorrect: true },
    { action: "Ignore Issues", emoji: "🙈", isCorrect: false },
    { action: "Try New Methods", emoji: "🧪", isCorrect: true },
    { action: "Stick to Old Ways", emoji: "📜", isCorrect: false },
    { action: "Think Outside Box", emoji: "📦", isCorrect: true },
    { action: "Follow the Crowd", emoji: "🐑", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for innovation choices...'}
              {gameState === 'showing' && 'Watch for the innovation signal...'}
              {gameState === 'ready' && 'QUICK! Choose the innovative approach!'}
              {gameState === 'success' && `Great innovation! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Choose innovation!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Innovative Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Missed Innovation!</h3>
                <p className="text-white/90">Remember: Choose creative and innovative approaches!</p>
              </div>
            </div>
          )}
        </div>
