...
const ReflexTeenMoney = () => {
  ...
  const scenarios = [
    { action: "Profit = Good", emoji: "💰", isCorrect: true },
    { action: "Loss = Happy", emoji: "😢", isCorrect: false },
    { action: "Budget = Smart", emoji: "📊", isCorrect: true },
    { action: "Overspend = Wise", emoji: "💸", isCorrect: false },
    { action: "Save = Secure", emoji: "🐷", isCorrect: true },
    { action: "Debt = Freedom", emoji: "⛓️", isCorrect: false },
    { action: "Invest = Grow", emoji: "📈", isCorrect: true },
    { action: "Waste = Success", emoji: "🗑️", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for money concepts...'}
              {gameState === 'showing' && 'Watch for the financial signal...'}
              {gameState === 'ready' && 'QUICK! Choose the correct financial concept!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Choose correct financial concepts!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Correct Financial Concept!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Incorrect Financial Concept!</h3>
                <p className="text-white/90">Remember: Choose correct financial principles!</p>
              </div>
            </div>
          )}
        </div>
