...
const ReflexTeenResponsibility = () => {
  ...
  const scenarios = [
    { action: "Eco-Friendly Business", emoji: "♻️", isCorrect: true },
    { action: "Polluting Business", emoji: "🏭", isCorrect: false },
    { action: "Ethical Practices", emoji: "✅", isCorrect: true },
    { action: "Exploitative Labor", emoji: "⚠️", isCorrect: false },
    { action: "Community Support", emoji: "🤝", isCorrect: true },
    { action: "Ignore Stakeholders", emoji: "🔇", isCorrect: false },
    { action: "Sustainable Growth", emoji: "🌱", isCorrect: true },
    { action: "Short-term Profits", emoji: "💸", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for responsibility decisions...'}
              {gameState === 'showing' && 'Watch for the business ethics signal...'}
              {gameState === 'ready' && 'QUICK! Choose the responsible business action!'}
              {gameState === 'success' && `Great ethics! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Responsible businesses need quick ethical decisions!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Excellent Ethics!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Ethics Missed!</h3>
                <p className="text-white/90">Remember: Responsible businesses prioritize ethics and sustainability!</p>
              </div>
            </div>
          )}
        </div>
