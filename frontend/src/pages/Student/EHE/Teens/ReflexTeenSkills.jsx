...
const ReflexTeenSkills = () => {
  ...
  const scenarios = [
    { action: "Leadership", emoji: "👑", isCorrect: true },
    { action: "Bullying", emoji: "😠", isCorrect: false },
    { action: "Teamwork", emoji: "🤝", isCorrect: true },
    { action: "Cheating", emoji: "🤥", isCorrect: false },
    { action: "Communication", emoji: "💬", isCorrect: true },
    { action: "Lying", emoji: "🐍", isCorrect: false },
    { action: "Problem-Solving", emoji: "🧩", isCorrect: true },
    { action: "Giving Up", emoji: "🏳️", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for skill matching...'}
              {gameState === 'showing' && 'Watch for the skill signal...'}
              {gameState === 'ready' && 'QUICK! Choose the positive skill!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Choose positive skills!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Positive Skill!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Negative Behavior!</h3>
                <p className="text-white/90">Remember: Choose positive entrepreneurial skills!</p>
              </div>
            </div>
          )}
        </div>
