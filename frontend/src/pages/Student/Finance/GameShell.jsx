          ...
          {!allAnswersCorrect && submissionComplete ? (
            ...
            <button
              onClick={async () => {
                ...
                let resolvedNextGamePath = nextGamePath || location.state?.nextGamePath;
                
                // If no next game path is available, try to determine it from current path
                if (!resolvedNextGamePath) {
                  console.log('⚠️ No nextGamePath found, trying to determine from current path:', location.pathname);
                  
                  // Try to get the current category and age group from the current path
                  const pathParts = location.pathname.split('/');
                  if (pathParts.length >= 4 && pathParts[1] === 'student') {
                    const category = pathParts[2];
                    const ageGroup = pathParts[3];
                    
                    // Attempt to determine next game by looking up game data
                    try {
                      // Import the appropriate game data based on category and age
                      let games = [];
                      if (category === 'finance' && ageGroup === 'kids') {
                        const { getFinanceKidsGames } = await import('../../../pages/Games/GameCategories/Finance/kidGamesData');
                        games = getFinanceKidsGames({});
                      } else if (category === 'finance' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getFinanceTeenGames } = await import('../../../pages/Games/GameCategories/Finance/teenGamesData');
                        games = getFinanceTeenGames({});
                      } else if (category === 'brain' && ageGroup === 'kids') {
                        const { getBrainKidsGames } = await import('../../../pages/Games/GameCategories/Brain/kidGamesData');
                        games = getBrainKidsGames({});
                      } else if (category === 'brain' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getBrainTeenGames } = await import('../../../pages/Games/GameCategories/Brain/teenGamesData');
                        games = getBrainTeenGames({});
                      } else if (category === 'uvls' && ageGroup === 'kids') {
                        const { getUvlsKidsGames } = await import('../../../pages/Games/GameCategories/UVLS/kidGamesData');
                        games = getUvlsKidsGames({});
                      } else if (category === 'uvls' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getUvlsTeenGames } = await import('../../../pages/Games/GameCategories/UVLS/teenGamesData');
                        games = getUvlsTeenGames({});
                      } else if (category === 'dcos' && ageGroup === 'kids') {
                        const { getDcosKidsGames } = await import('../../../pages/Games/GameCategories/DCOS/kidGamesData');
                        games = getDcosKidsGames({});
                      } else if (category === 'dcos' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getDcosTeenGames } = await import('../../../pages/Games/GameCategories/DCOS/teenGamesData');
                        games = getDcosTeenGames({});
                      } else if (category === 'moral-values' && ageGroup === 'kids') {
                        const { getMoralKidsGames } = await import('../../../pages/Games/GameCategories/MoralValues/kidGamesData');
                        games = getMoralKidsGames({});
                      } else if (category === 'moral-values' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getMoralTeenGames } = await import('../../../pages/Games/GameCategories/MoralValues/teenGamesData');
                        games = getMoralTeenGames({});
                      } else if (category === 'ai-for-all' && ageGroup === 'kids') {
                        const { getAiKidsGames } = await import('../../../pages/Games/GameCategories/AiForAll/kidGamesData');
                        games = getAiKidsGames({});
                      } else if (category === 'ai-for-all' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getAiTeenGames } = await import('../../../pages/Games/GameCategories/AiForAll/teenGamesData');
                        games = getAiTeenGames({});
                      } else if (category === 'ehe' && ageGroup === 'kids') {
                        const { getEheKidsGames } = await import('../../../pages/Games/GameCategories/EHE/kidGamesData');
                        games = getEheKidsGames({});
                      } else if (category === 'ehe' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getEheTeenGames } = await import('../../../pages/Games/GameCategories/EHE/teenGamesData');
                        games = getEheTeenGames({});
                      } else if (category === 'civic-responsibility' && ageGroup === 'kids') {
                        const { getCrgcKidsGames } = await import('../../../pages/Games/GameCategories/CRGC/kidGamesData');
                        games = getCrgcKidsGames({});
                      } else if (category === 'civic-responsibility' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getCrgcTeensGames } = await import('../../../pages/Games/GameCategories/CRGC/teenGamesData');
                        games = getCrgcTeensGames({});
                      } else if (category === 'health-male' && ageGroup === 'kids') {
                        const { getHealthMaleKidsGames } = await import('../../../pages/Games/GameCategories/HealthMale/kidGamesData');
                        games = getHealthMaleKidsGames({});
                      } else if (category === 'health-male' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getHealthMaleTeenGames } = await import('../../../pages/Games/GameCategories/HealthMale/teenGamesData');
                        games = getHealthMaleTeenGames({});
                      } else if (category === 'health-female' && ageGroup === 'kids') {
                        const { getHealthFemaleKidsGames } = await import('../../../pages/Games/GameCategories/HealthFemale/kidGamesData');
                        games = getHealthFemaleKidsGames({});
                      } else if (category === 'health-female' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getHealthFemaleTeenGames } = await import('../../../pages/Games/GameCategories/HealthFemale/teenGamesData');
                        games = getHealthFemaleTeenGames({});
                      } else if (category === 'sustainability' && ageGroup === 'kids') {
                        const { getSustainabilityKidsGames } = await import('../../../pages/Games/GameCategories/Sustainability/kidGamesData');
                        games = getSustainabilityKidsGames({});
                      } else if (category === 'sustainability' && (ageGroup === 'teens' || ageGroup === 'teen')) {
                        const { getSustainabilityTeenGames } = await import('../../../pages/Games/GameCategories/Sustainability/teenGamesData');
                        games = getSustainabilityTeenGames({});
                      }
                      ...
                      if (games.length > 0) {
                        ...
                        if (currentGame && currentGame.index !== undefined) {
                          ...
                          if (nextGame) {
                            resolvedNextGamePath = nextGame.path;
                            console.log('🎮 Determined next game path:', resolvedNextGamePath);
                          }
                        }
                      }
                    } catch (error) {
                      console.error('Error determining next game path:', error);
                    }
                  }
                }
                ...
                try {
                  ...
                  let resolvedNextGameId = nextGameId || location.state?.nextGameId || null;
                  
                  // If nextGameId is not available, try to determine it from the path
                      ...
                      try {
                        ...
                        if (games.length > 0) {
                          ...
                          if (currentGame && currentGame.index !== undefined) {
                            ...
                            const nextGame = games.find(g => g.index === currentGame.index + 1 && g.isSpecial && g.path);
                            if (nextGame) {
                              resolvedNextGameId = nextGame.id;
                              console.log('🎮 Determined next game ID:', resolvedNextGameId);
                            }
                          }
                        }
                      } catch (error) {
                        console.error('Error determining next game ID:', error);
                      }
                    }
                  }
