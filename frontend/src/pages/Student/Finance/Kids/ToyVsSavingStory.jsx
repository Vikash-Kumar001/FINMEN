...
const ToyVsSavingStory = () => {
  ...
  const stages = [
    {
      id: 1,
      action: "Save for Bicycle",
      wrong: "Buy Toy Today",
      prompt: "You really want a new toy, but you also dream of a bicycle. What is smarter?",
    },
    {
      id: 2,
      action: "Save Gift Money",
      wrong: "Spend All Now",
      prompt: "You get gift money on your birthday. How should you use it?",
    },
    {
      id: 3,
      action: "Build Savings First",
      wrong: "Buy New Gadget",
      prompt: "There is a new gadget you love. What is the best money move?",
    },
    {
      id: 4,
      action: "Save Allowance",
      wrong: "Buy Candy",
      prompt: "You get weekly allowance. What helps your future goals most?",
    },
    {
      id: 5,
      action: "Think Long-Term",
      wrong: "Quick Spend",
      prompt: "A cool toy is on sale today. What should you focus on?",
    },
  ];
  ...
  return (
    <div className="h-screen w-full bg-gradient-to-br from-pink-100 via-rose-50 to-indigo-100 flex flex-col relative overflow-hidden">
      ...
      <div className="flex items-center justify-between px-2 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 relative z-30 bg-white/40 backdrop-blur-sm border-b border-pink-200 flex-shrink-0 gap-2 sm:gap-4">
        ...
        <div className="flex-1 flex items-center justify-center min-w-0">
          <h1 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold px-2 flex items-center justify-center gap-1 sm:gap-2 truncate">
            <Gift className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-pink-500" />
            <span className="bg-gradient-to-r from-pink-600 via-rose-600 to-indigo-600 bg-clip-text text-transparent truncate">
              <span className="hidden xs:inline">Toy vs Saving Story</span>
              <span className="xs:hidden">Toy vs Saving</span>
            </span>
          </h1>
        </div>
          ...
          {finalScore === 0 ? (
            !showResult ? (
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="bg-white/95 backdrop-blur-md rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 border-2 border-pink-200 shadow-xl">
                  <div className="flex items-center justify-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                    <Gift className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-pink-500" />
                    <span className="text-xs sm:text-sm md:text-base font-semibold text-pink-700">
                      Saving Choice {currentStage + 1}
                    </span>
                  </div>
              ...
              <div className="bg-white/95 backdrop-blur-md rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 border-2 border-pink-200 shadow-xl text-center">
                ...
                <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold text-pink-800 mb-2 sm:mb-4">
                  {isCorrect ? "Smart Saver!" : "Think Long-Term"}
                </h3>
                <p className="text-lg sm:text-xl text-pink-900 mb-6 sm:mb-8 leading-relaxed px-1">
                  {isCorrect
                    ? "Great choice! Saving now helps you reach bigger goals later."
                    : "A better choice is: " + stages[currentStage].action}
                </p>
                <button
                  onClick={handleNext}
                ...
                >
                  {currentStage < stages.length - 1 ? "Next Story Choice" : "Finish"}
                </button>
              </div>
            )
          ) : (
            <div className="bg-white/95 backdrop-blur-md rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 border-2 border-pink-200 shadow-xl text-center max-w-2xl w-full">
              ...
              <p className="text-pink-900 text-xs sm:text-sm md:text-base mb-3 sm:mb-4 leading-relaxed px-1">
                You scored {finalScore} out of {stages.length} — you’re great at choosing
                saving over quick toys!
                <br />
                Big goals like cycles and trips become possible when you save instead of
                spending right away. 🚲
              </p>
              ...
              <p className="text-pink-800 text-xs sm:text-sm mb-3 sm:mb-4 px-1">
                Lesson: Saving today means more choices and bigger dreams tomorrow.
              </p>
              {allStagesAnswered && (
                <button
                  onClick={() => navigate(resolvedBackPath)}
                  className="bg-gradient-to-r from-pink-500 via-rose-500 to-indigo-500 hover:from-pink-600 hover:via-rose-600 hover:to-indigo-600 text-white py-2 sm:py-2.5 px-4 sm:px-6 rounded-full font-bold text-xs sm:text-sm md:text-base shadow-lg transition-all transform hover:scale-105"
                >
