...
const JournalConsumerRights = () => {
  const navigate = useNavigate();
  const location = useLocation();
  
  // Get game data from game category folder (source of truth)
  const gameId = "finance-teens-177";
  ...
  const stages = [
    { id: 1, prompt: "One right I know as a consumer is ___." },
    { id: 2, prompt: "One way to protect my consumer rights is ___." },
    { id: 3, prompt: "One place to report consumer fraud is ___." },
    { id: 4, prompt: "One thing to check before buying is ___." },
    { id: 5, prompt: "One benefit of consumer rights is ___." }
  ];
      ...
      <div className="text-white space-y-8">
        {!showResult ? (
          ...
          <div className="bg-white/10 backdrop-blur-md p-8 rounded-2xl border border-white/20 text-center">
            ...
            <h3 className="text-3xl font-bold mb-4">Consumer Rights Journal Star!</h3>
            <p className="text-white/90 text-lg mb-6">You completed {score} out of 5 stages!</p>
            <div className="bg-green-500 py-3 px-6 rounded-full inline-flex items-center gap-2">
              +{score} Coins
            </div>
            <p className="text-white/80 mt-4">Lesson: Know your rights as a consumer!</p>
          </div>
        )}
      </div>
    </GameShell>
...
export default JournalConsumerRights;