...
const NutritionMatchPuzzle = () => {
  ...
  const items = [
    { id: 'milk', emoji: '🥛', match: 'calcium', description: 'Milk is rich in calcium for strong bones' },
    { id: 'calcium', emoji: '🦴', match: 'milk', description: 'Calcium makes your bones and teeth strong' },
    { id: 'rice', emoji: '🍚', match: 'carbs', description: 'Rice gives you energy through carbohydrates' },
    { id: 'carbs', emoji: '⚡', match: 'rice', description: 'Carbohydrates are your body\'s main energy source' },
    { id: 'apple', emoji: '🍎', match: 'vitamins', description: 'Apples are packed with vitamins to keep you healthy' },
    { id: 'vitamins', emoji: '💊', match: 'apple', description: 'Vitamins help your body fight diseases' },
    { id: 'dal', emoji: '🍛', match: 'protein', description: 'Dal is a great source of plant-based protein' },
    { id: 'protein', emoji: '💪', match: 'dal', description: 'Protein helps build and repair muscles' },
    { id: 'spinach', emoji: '🍃', match: 'iron', description: 'Spinach is rich in iron to prevent anemia' },
    { id: 'iron', emoji: '🧲', match: 'spinach', description: 'Iron helps carry oxygen in your blood' }
  ];
                ...
                {isFlipped ? (
                  <div className="text-center p-2 text-white">
                    ...
                    <div className="text-xs font-medium bg-white/20 px-2 py-1 rounded-full">
                      {item.id === 'milk' ? 'Milk' : 
                       item.id === 'calcium' ? 'Calcium' :
                       item.id === 'rice' ? 'Rice' :
                       item.id === 'carbs' ? 'Carbs' :
                       item.id === 'apple' ? 'Apple' :
                       item.id === 'vitamins' ? 'Vitamins' :
                       item.id === 'dal' ? 'Dal' :
                       item.id === 'protein' ? 'Protein' :
                       item.id === 'spinach' ? 'Spinach' : 'Iron'}
                    </div>
                  </div>
                ) : (
