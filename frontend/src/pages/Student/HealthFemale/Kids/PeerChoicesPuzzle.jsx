...
const PeerChoicesPuzzle = () => {
  ...
  const items = [
    { id: 'sayno', emoji: '🙅‍♀️', match: 'safe', description: 'Saying no to peer pressure keeps you safe' },
    { id: 'safe', emoji: '🛡️', match: 'sayno', description: 'Safety comes from making good choices' },
    { id: 'agree', emoji: '✅', match: 'trouble', description: 'Agreeing to wrong things can lead to trouble' },
    { id: 'trouble', emoji: '⚠️', match: 'agree', description: 'Trouble often comes from poor choices' },
    { id: 'askhelp', emoji: '🙋‍♀️', match: 'support', description: 'Asking for help gets you the support you need' },
    { id: 'support', emoji: '🤝', match: 'askhelp', description: 'Support helps you make better decisions' },
    { id: 'goodfriends', emoji: '👥', match: 'encourage', description: 'Good friends encourage positive choices' },
    { id: 'encourage', emoji: '💪', match: 'goodfriends', description: 'Encouragement helps you stay strong' },
    { id: 'standup', emoji: '👩‍🦱', match: 'respect', description: 'Standing up for yourself earns respect' },
    { id: 'respect', emoji: '✨', match: 'standup', description: 'Respect comes from being true to yourself' }
  ];
                ...
                {isFlipped ? (
                  <div className="text-center p-2 text-white">
                    ...
                    <div className="text-xs font-medium bg-white/20 px-2 py-1 rounded-full">
                      {item.id === 'sayno' ? 'Say No' : 
                       item.id === 'safe' ? 'Safe' :
                       item.id === 'agree' ? 'Agree' :
                       item.id === 'trouble' ? 'Trouble' :
                       item.id === 'askhelp' ? 'Ask Help' :
                       item.id === 'support' ? 'Support' :
                       item.id === 'goodfriends' ? 'Good Friends' :
                       item.id === 'encourage' ? 'Encourage' :
                       item.id === 'standup' ? 'Stand Up' : 'Respect'}
                    </div>
                  </div>
                ) : (
