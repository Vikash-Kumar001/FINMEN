...
const SafetyToolsPuzzle = () => {
  ...
  const items = [
    { id: 'seatbelt', emoji: '🪑', match: 'car', description: 'Seatbelts keep you safe in cars' },
    { id: 'car', emoji: '🚗', match: 'seatbelt', description: 'Cars need seatbelts for passenger safety' },
    { id: 'helmet', emoji: '⛑️', match: 'bike', description: 'Helmets protect your head when riding' },
    { id: 'bike', emoji: '🚲', match: 'helmet', description: 'Bikes require helmets for head protection' },
    { id: 'soap', emoji: '🧼', match: 'hands', description: 'Soap cleans your hands of germs' },
    { id: 'hands', emoji: '✋', match: 'soap', description: 'Hands need soap for proper cleaning' },
    { id: 'sunscreen', emoji: '🧴', match: 'skin', description: 'Sunscreen protects your skin from UV rays' },
    { id: 'skin', emoji: '☀️', match: 'sunscreen', description: 'Skin needs sunscreen for sun protection' },
    { id: 'mask', emoji: '😷', match: 'pollution', description: 'Masks protect from air pollution' },
    { id: 'pollution', emoji: '🌫️', match: 'mask', description: 'Pollution requires masks for protection' }
  ];
                ...
                {isFlipped ? (
                  <div className="text-center p-2 text-white">
                    ...
                    <div className="text-xs font-medium bg-white/20 px-2 py-1 rounded-full">
                      {item.id === 'seatbelt' ? 'Seatbelt' : 
                       item.id === 'car' ? 'Car' :
                       item.id === 'helmet' ? 'Helmet' :
                       item.id === 'bike' ? 'Bike' :
                       item.id === 'soap' ? 'Soap' :
                       item.id === 'hands' ? 'Hands' :
                       item.id === 'sunscreen' ? 'Sunscreen' :
                       item.id === 'skin' ? 'Skin' :
                       item.id === 'mask' ? 'Mask' : 'Pollution'}
                    </div>
                  </div>
                ) : (
