...
const ReflexPubertyBasics = () => {
  ...
  const scenarios = [
    { action: "Normal Changes", emoji: "✅", isCorrect: true },
    { action: "Shame", emoji: "❌", isCorrect: false },
    { action: "Natural Process", emoji: "🌱", isCorrect: true },
    { action: "Embarrassment", emoji: "😳", isCorrect: false },
    { action: "Growth", emoji: "📏", isCorrect: true },
    { action: "Weirdness", emoji: "👽", isCorrect: false },
    { action: "Development", emoji: "🦋", isCorrect: true },
    { action: "Abnormality", emoji: "⚠️", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for smart puberty choices...'}
              {gameState === 'showing' && 'Watch for the puberty signal...'}
              {gameState === 'ready' && 'QUICK! Choose the healthy attitude!'}
              {gameState === 'success' && `Smart choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose positive attitudes!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Smart Attitude!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose positive attitudes!</p>
              </div>
            </div>
          )}
        </div>
