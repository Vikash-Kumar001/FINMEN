...
const ReflexTeenAlert = () => {
  ...
  const scenarios = [
    { action: "Take Medicine as Advised", emoji: "💊", isCorrect: true },
    { action: "Skip Dose", emoji: "❌", isCorrect: false },
    { action: "Follow Prescription", emoji: "📋", isCorrect: true },
    { action: "Ignore Instructions", emoji: "🙈", isCorrect: false },
    { action: "Complete Treatment", emoji: "✅", isCorrect: true },
    { action: "Stop Early", emoji: "🛑", isCorrect: false },
    { action: "Track Symptoms", emoji: "📊", isCorrect: true },
    { action: "Disregard Changes", emoji: "🤔", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for medication choices...'}
              {gameState === 'showing' && 'Watch for the health signal...'}
              {gameState === 'ready' && 'QUICK! Choose the positive medication habit!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always follow medical advice!'}
            </p>
          </div>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always follow medical advice!</p>
              </div>
            </div>
          )}
        </div>
