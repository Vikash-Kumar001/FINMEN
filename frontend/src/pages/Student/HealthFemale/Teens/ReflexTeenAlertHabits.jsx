...
const ReflexTeenAlertHabits = () => {
  ...
  const scenarios = [
    ...
    { action: "Skipping Habits", emoji: "❌", isCorrect: false },
    { action: "Daily Practice", emoji: "📅", isCorrect: true },
    { action: "Occasional Effort", emoji: "⏱️", isCorrect: false },
    { action: "Mindful Routine", emoji: "🧘", isCorrect: true },
    { action: "Random Actions", emoji: "🔀", isCorrect: false },
    { action: "Goal Focus", emoji: "🎯", isCorrect: true },
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for habit consistency choices...'}
              {gameState === 'showing' && 'Watch for the consistency signal...'}
              {gameState === 'ready' && 'QUICK! Choose the consistency habit!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose consistency!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Consistent Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Inconsistent Choice!</h3>
                <p className="text-white/90">Remember: Always choose consistency!</p>
              </div>
            </div>
          )}
        </div>
