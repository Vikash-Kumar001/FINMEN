...
const ReflexTeenEsteem = () => {
  ...
  const scenarios = [
    { action: "Respect Myself", emoji: "❤️", isCorrect: true },
    { action: "Put Myself Down", emoji: "💔", isCorrect: false },
    { action: "Value My Opinions", emoji: "💭", isCorrect: true },
    { action: "Ignore My Thoughts", emoji: "🤫", isCorrect: false },
    { action: "Stand Up for Myself", emoji: "✊", isCorrect: true },
    { action: "Let Others Walk Over Me", emoji: "🦶", isCorrect: false },
    { action: "Celebrate My Wins", emoji: "🎉", isCorrect: true },
    { action: "Focus Only on Failures", emoji: "😞", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for self-respect choices...'}
              {gameState === 'showing' && 'Watch for the self-esteem signal...'}
              {gameState === 'ready' && 'QUICK! Choose the positive self-respect option!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose positive self-respect!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Positive Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose positive self-respect!</p>
              </div>
            </div>
          )}
        </div>
