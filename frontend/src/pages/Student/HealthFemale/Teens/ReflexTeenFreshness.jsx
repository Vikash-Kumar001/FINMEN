...
const ReflexTeenFreshness = () => {
  ...
  const scenarios = [
    ...
    { action: "New Pad", emoji: "🩹", isCorrect: true },
    { action: "Old Pad", emoji: "📎", isCorrect: false },
    { action: "Clean Sheets", emoji: "🛏️", isCorrect: true },
    { action: "Dirty Sheets", emoji: "🛋️", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for freshness choices...'}
              {gameState === 'showing' && 'Watch for the freshness signal...'}
              {gameState === 'ready' && 'QUICK! Choose the fresh option!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose freshness!'}
            </p>
          </div>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose freshness!</p>
              </div>
            </div>
          )}
        </div>
