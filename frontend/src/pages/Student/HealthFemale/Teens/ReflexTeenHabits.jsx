...
const ReflexTeenHabits = () => {
  ...
  const scenarios = [
    { action: "Exercise Daily", emoji: "🏃‍♀️", isCorrect: true },
    { action: "Ignore Health", emoji: "❌", isCorrect: false },
    { action: "Drink Water", emoji: "💧", isCorrect: true },
    { action: "Skip Meals", emoji: "🍽️", isCorrect: false },
    { action: "Early Sleep", emoji: "😴", isCorrect: true },
    { action: "All Nighter", emoji: "🌙", isCorrect: false },
    { action: "Study Time", emoji: "📚", isCorrect: true },
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for healthy habit choices...'}
              {gameState === 'showing' && 'Watch for the habit signal...'}
              {gameState === 'ready' && 'QUICK! Choose the healthy habit!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose healthy habits!'}
            </p>
          </div>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose healthy habits!</p>
              </div>
            </div>
          )}
        </div>
