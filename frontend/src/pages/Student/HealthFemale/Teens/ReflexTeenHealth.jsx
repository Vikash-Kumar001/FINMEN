...
const ReflexPreventiveCare = () => {
  ...
  const scenarios = [
    ...
    { action: "Take Medicine", emoji: "💊", isCorrect: true },
    { action: "Skip Dose", emoji: "🚫", isCorrect: false },
    { action: "Healthy Diet", emoji: "🥗", isCorrect: true },
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for health choices...'}
              {gameState === 'showing' && 'Watch for the health signal...'}
              {gameState === 'ready' && 'QUICK! Choose the positive health option!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose positive health habits!'}
            </p>
          </div>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose positive health habits!</p>
              </div>
            </div>
          )}
        </div>
