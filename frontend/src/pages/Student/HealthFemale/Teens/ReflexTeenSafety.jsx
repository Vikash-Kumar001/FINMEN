...
const ReflexTeenSafety = () => {
  ...
  const scenarios = [
    ...
    { action: "Use Substances", emoji: "❌", isCorrect: false },
    { action: "Call Trusted Adult", emoji: "📞", isCorrect: true },
    { action: "Ignore Danger", emoji: "🙈", isCorrect: false },
    { action: "Leave Situation", emoji: "🚪", isCorrect: true },
    { action: "Stay in Danger", emoji: "⚠️", isCorrect: false },
    { action: "Make Safe Choice", emoji: "✅", isCorrect: true },
    { action: "Risk Safety", emoji: "❗", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Get ready for safety choices...'}
              {gameState === 'showing' && 'Watch for the safety signal...'}
              {gameState === 'ready' && 'QUICK! Choose the safe action!'}
              {gameState === 'success' && `Great choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose safe actions!'}
            </p>
          </div>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                <div className="text-6xl">{currentScenario.find(s => !s.isCorrect)?.emoji}</div>
                <h3 className="text-xl font-bold text-white">Unsafe Choice!</h3>
                <p className="text-white/90">Remember: Always choose safe actions!</p>
              </div>
            </div>
          )}
        </div>
