...
const ReflexGrowthAlert = () => {
  ...
  const scenarios = [
    { action: "🏃 Exercise Daily", emoji: "🏃", isCorrect: true },
    { action: "❌ No Exercise", emoji: "❌", isCorrect: false },
    { action: "🥛 Drink Milk", emoji: "🥛", isCorrect: true },
    { action: "🥤 Skip Milk", emoji: "🥤", isCorrect: false },
    { action: "🛌 Good Sleep", emoji: "🛌", isCorrect: true },
    { action: "📱 No Sleep", emoji: "📱", isCorrect: false },
    { action: "🥕 Healthy Food", emoji: "🥕", isCorrect: true },
    { action: "🍔 Junk Food", emoji: "🍔", isCorrect: false }
  ];
  ...
  const handleNext = () => {
    navigate("/student/health-male/kids/growing-strong-badge");
  };
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Growth alert incoming...'}
              {gameState === 'showing' && 'Watch for the growth signal...'}
              {gameState === 'ready' && 'QUICK! Choose the healthy growth choice!'}
              {gameState === 'success' && `Growth choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose healthy growth!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Growth Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose healthy growth habits!</p>
              </div>
            </div>
          )}
        </div>
