...
const ReflexHygieneCheck = () => {
  ...
  const scenarios = [
    { action: "🧼 Soap", emoji: "🧼", isCorrect: true },
    { action: "❌ Skip Bath", emoji: "❌", isCorrect: false },
    { action: "🧴 Deodorant", emoji: "🧴", isCorrect: true },
    { action: "🦨 No Deodorant", emoji: "🦨", isCorrect: false },
    { action: "🧺 Clean Clothes", emoji: "🧺", isCorrect: true },
    { action: "👕 Dirty Shirt", emoji: "👕", isCorrect: false },
    { action: "🚿 Shower", emoji: "🚿", isCorrect: true },
    { action: "😴 Skip Shower", emoji: "😴", isCorrect: false }
  ];
  ...
  const handleNext = () => {
    navigate("/student/health-male/kids/hygiene-match-puzzle");
  };
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Hygiene check incoming...'}
              {gameState === 'showing' && 'Watch for the hygiene signal...'}
              {gameState === 'ready' && 'QUICK! Choose the hygienic option!'}
              {gameState === 'success' && `Hygiene choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose good hygiene!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Hygiene Choice!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose good hygiene habits!</p>
              </div>
            </div>
          )}
        </div>
