...
const ReflexPreventiveHealth = () => {
  ...
  const scenarios = [
    { action: "Regular Checkups", emoji: "🏥", isCorrect: true },
    { action: "Avoid Doctor", emoji: "❌", isCorrect: false },
    { action: "Get Vaccines", emoji: "💉", isCorrect: true },
    { action: "Skip Shots", emoji: "😬", isCorrect: false },
    { action: "Healthy Eating", emoji: "🥗", isCorrect: true },
    { action: "Junk Food Only", emoji: "🍕", isCorrect: false },
    { action: "Exercise Daily", emoji: "🏃", isCorrect: true },
    { action: "Stay Sedentary", emoji: "🛋️", isCorrect: false }
  ];
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 relative">
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-white/90 text-lg">
              {gameState === 'waiting' && 'Preventive health scenario loading...'}
              {gameState === 'showing' && 'Watch for the health signal...'}
              {gameState === 'ready' && 'QUICK! Choose the preventive health action!'}
              {gameState === 'success' && `Healthy choice! (${reactionTime}ms)`}
              {gameState === 'fail' && 'Too slow! Always choose preventive health!'}
            </p>
          </div>
          ...
          {gameState === 'success' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-green-400">
                ...
                <h3 className="text-xl font-bold text-white">Preventive Health!</h3>
          ...
          {gameState === 'fail' && currentScenario && (
            <div className="text-center space-y-4">
              <div className="text-red-400">
                ...
                <p className="text-white/90">Remember: Always choose preventive health actions!</p>
              </div>
            </div>
          )}
        </div>
