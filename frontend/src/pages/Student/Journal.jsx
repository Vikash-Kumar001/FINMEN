...
export default function Journal() {
    ...
    useEffect(() => {
        if (socket && user) {
            ...
            socket.on('student:journal:data', setEntries);
            socket.on('student:journal:update', update => setEntries(prev => [update, ...prev]));
            socket.on('student:journal:refresh', setEntries);
            return () => {
    ...
    const handleSaveEntry = () => {
        ...
        if (editingEntry) {
            socket.emit('student:journal:update', { studentId: user._id, entryId: editingEntry._id, payload });
        } else {
            socket.emit('student:journal:create', { studentId: user._id, payload });
        }
        setDraft("");
        setTitle("");
        setTags("");
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this entry?")) {
            socket.emit('student:journal:delete', { studentId: user._id, entryId: id });
        }
    };
