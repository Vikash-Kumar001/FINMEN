...
const BadgeTruthfulKid = () => {
  ...
  // ✅ 5 Honesty Acts (Yes/No)
  const honestyActs = [
    { id: 1, text: "Returned a lost pencil", emoji: "✏️" },
    { id: 2, text: "Admitted not doing homework", emoji: "📚" },
    { id: 3, text: "Did not cheat during an exam", emoji: "🚫" },
    { id: 4, text: "Returned extra candy", emoji: "🍬" },
    { id: 5, text: "Always chose truth over lies", emoji: "💎" },
  ];
  ...
  // ✅ Submit Logic
  const handleSubmit = () => {
    if (Object.keys(answers).length !== honestyActs.length) {
      alert("Please answer all honesty acts before submitting!");
      return;
    }

    const allYes = honestyActs.every((act) => answers[act.id] === "yes");
    setIsWinner(allYes);
    setShowResult(true);
    ...
    >
      <div className="space-y-6">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20">
          <h2 className="text-2xl font-bold text-white mb-6 text-center">
            Truthful Kid Challenge: Be Honest Always 💎
          </h2>

          <p className="text-white/80 mb-6 text-center">
            Answer honestly — do you practice truth in daily life?
          </p>
            ...
            <div className="mt-8 text-center">
              {isWinner ? (
                <div className="text-green-400 text-xl font-bold">
                  💎 Honest and Proud! You are a Truthful Kid!
                </div>
              ) : (
                <div className="text-red-400 text-lg font-semibold">
                  ⚠️ Some answers show dishonesty — try again with full truth!
                </div>
              )}
            </div>
          )}
      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-50">
          <div className="bg-gradient-to-r from-blue-400 via-cyan-400 to-teal-400 text-white rounded-2xl p-10 text-center shadow-2xl animate-bounce">
            <div className="text-6xl mb-4">🏆</div>
            <h3 className="text-3xl font-bold mb-2">Congratulations!</h3>
            <p className="text-lg mb-6">
              You’ve earned the <strong>Truthful Kid Badge!</strong> 💎
            </p>
            <button
