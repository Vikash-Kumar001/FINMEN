...
const items = [
  { word: "BANANA PEEL", isCorrect: true },
  { word: "PLASTIC BOTTLE", isCorrect: false },
  { word: "APPLE CORE", isCorrect: true },
  { word: "GLASS JAR", isCorrect: false },
  { word: "VEGETABLE SCRAPS", isCorrect: true },
  { word: "METAL CAN", isCorrect: false },
  { word: "COFFEE GROUNDS", isCorrect: true },
...
const ReflexCompostMaster = () => {
  ...
  }, [gameState, currentItem, showNextItem, showCorrectAnswerFeedback]);

  return (
    <GameShell
      title="Reflex Compost Master"
      score={score}
      subtitle={gameState === "ready" ? "Tap on compostable items quickly!" : gameState === "playing" ? `Round ${currentRound} of ${TOTAL_ROUNDS}` : "Game Over!"}
      ...
      <div className="space-y-8">
        ...
        {gameState === "playing" && currentItem && (
          <div onClick={handleTap} className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 cursor-pointer">
            <div className="text-4xl font-bold text-white mb-4">{currentItem.word}</div>
            <p className="text-lg text-white/80">Time Left: {timeLeft}s</p>
            <p className="text-sm text-white/60 mt-2">Tap if it's compostable!</p>
          </div>
        )}
      </div>
    </GameShell>
